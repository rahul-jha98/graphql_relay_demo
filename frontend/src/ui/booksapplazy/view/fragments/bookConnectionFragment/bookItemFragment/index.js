...
export default ({ bookNodeRef}) => {
    const book = useFragment(graphql`
        fragment bookItemFragment2Fragment on Book
            @argumentDefinitions(
            ...
            )
        {
            id
            name
            ...bookMetaData2Fragment @arguments(fetchAuthorName: $fetchAuthorName)

            comments (first: 1) @include(if: $includeRecentComments) {
                edges {
                    cursor
                }
                ...commentEdgesFragment2Fragment @arguments(fetchBookDetail: false, skipUser: true, skipTimestamp: true)
            }
        }
    `, bookNodeRef);

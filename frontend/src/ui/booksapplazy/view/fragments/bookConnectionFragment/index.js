...
export default ({ rootRef, title, showButtonForComments }) => {
    const {data, hasNext, loadNext, isLoadingNext, refetch} = usePaginationFragment(graphql`
        fragment bookConnectionFragment2Fragment on Query @refetchable(queryName: "BooksPagintaionQuery2")
        @argumentDefinitions(
            ...
            books(first: $first, after: $after, author_id: $author_id) @connection(key: "PaginatedList2_books") {
                edges {
                    node {
                        id
                        ...bookItemFragment2Fragment @arguments(fetchAuthorName: $fetchAuthorName, includeRecentComments: $includeRecentComments)
                    }
                }
            }
        }
